...
input.addEventListener("change", async () => {
  ...
  processingStatus.innerText =
    "File selected. Click 'Upload to AI' to process your image.";
});
...
function processFile(file) {
  // as the filereader API is asynchronous, we need to return a promise to handle the result
...
async function makeApiRequest(userImageURL) {
  try {
    ...
    const selectedStyle = document.querySelector(
      '.user-prompt input[name="user-prompt"]:checked'
    ).value;
    ...
    if (selectedStyle === "minimalist") {
      ...
      userPrompt =
        "impressionist painting, capturing the essence of brushstrokes and color blending. Apply a painterly effect that emulates the style of artists like Monet or Van Gogh. Focus on creating a vibrant and textured appearance";
    } else if (selectedStyle === "disney") {
      userPrompt =
        "animated princess look in the style of Disney's Elsa of Frozen. Colours should be bright and playful.";
    }

    console.log("User's prompt:", userPrompt);

    // define the parameters for the API request eg what data to send to the API
    // TODO: replace with a free CROS proxy server if I can find one that works
    // const APIEndpoint =
    ...
    const dataRequestObject = {
      ...
      input: {
        ...
        guidance_scale: 5,
        negative_prompt:
          "(lowres, low quality, worst quality:1.2), (text:1.2), watermark, glitch, deformed, mutated, cross-eyed, ugly, disfigured,plain",
    ...
    if (data.status === "succeeded" && data.output && data.output.length > 0) {
      ...
      while (attempt < maxAttempts) {
        ...
        processingStatus.innerText =
          "Instant-ID is working hard to process your image...";
